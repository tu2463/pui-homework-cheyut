...
localStorage.setItem('curPage', 'detail');
...
function glazingChange() {
    ...
    glazingPrice = detail_allGlazing[glazingIndex].price;
    
    rollGlazing = detail_allGlazing[glazingIndex].name;
...
function packChange() {
    ...
    packPrice = detail_allPack[packIndex].price;
    packSize = detail_allPack[packIndex].size;
    priceChange(glazingPrice, packPrice);
  }
...
function priceChange(glazingPrice, packPrice) {
    let calculatedPrice = calculatePrice(basePrice, glazingPrice, packPrice);
...
for (let i = 0; i < Object.keys(detail_allGlazing).length; i++) {
    var option = document.createElement('option');
    option.text = detail_allGlazing[i].name;
    option.value = i;
    glazingSelect.add(option);
}

for (let i = 0; i < Object.keys(detail_allPack).length; i++) {
    var option = document.createElement('option');
    option.text = detail_allPack[i].size;
    option.value = i;
